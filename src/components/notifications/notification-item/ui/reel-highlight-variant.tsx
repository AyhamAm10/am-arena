...
export function createReelHighlightVariant(instanceId: string) {
  return function ReelHighlightVariant() {
    const byId = useMirror("byId");
    const item = byId[instanceId];
    const router = useRouter();
    const reelId = getNum(item?.data ?? null, "reelId");
    const markAsRead = item?.markAsRead;

    const go = useCallback(async () => {
      if (reelId == null || !item) return;
      navigateFromNotificationPayload(router, item.type, item.data);
      if (!item.read_at && markAsRead) {
        await markAsRead(item.id);
      }
    }, [item, markAsRead, reelId, router]);
    ...
    return (
      <MotionPressable
        ...
        onPress={() => void go()}
        disabled={reelId == null}
      >
        <Text style={s.title}>{item.title}</Text>
        <Text style={s.body}>{item.body}</Text>
        <ReadBadge readAt={item.read_at} />
        <Text style={s.meta}>{formatNotificationTime(item.created_at)} · فتح الريل</Text>
      </MotionPressable>
    );
  };
}
