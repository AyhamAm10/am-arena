...
export function createGlobalPollVariant(instanceId: string) {
  return function GlobalPollVariant() {
    ...
    return (
      <MotionPressable style={notificationCardStateStyle(item.read_at)} onPress={() => void go()}>
        ...
        <Text style={s.meta}>{formatNotificationTime(item.created_at)} · الانتقال للتصويت</Text>
      </MotionPressable>
    );
  };
}
